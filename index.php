...
<div style="position: relative;">
		   <img src="images/6876899.jpg" style="z-index:-1;border-radius: 0px 0px 10px 10px;object-fit: cover;" height='800hv' width='100%' max-width='100%' max-height='100%' id="target-section" />
  ...
  <div class="shop-header">
    ...
    <p style="font-size: 0.85rem; color: white; font-weight: 500; max-width: 700px; margin: 0 auto;">
      disclaim : we do not control the quality and stocks of those items, all of those come from our community (include you).
  ...
  <?php if ($result && $result->num_rows > 0): ?>
    <div class="product-grid">
      <?php while($row = $result->fetch_assoc()):
        ...
        $pictureFilename = basename($row['Picture']); // ensure it's just the file name
        $imagePath = 'images/' . $pictureFilename;
        $serverPath = __DIR__ . '/' . $imagePath;
        $image = (file_exists($serverPath) && !empty($pictureFilename)) ? $imagePath : $defaultImage;     
      ?>
